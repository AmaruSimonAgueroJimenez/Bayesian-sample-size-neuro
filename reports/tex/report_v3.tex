...
\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{fig_pca_swan.png}
...
\begin{figure}[H]
\centering\includegraphics[width=0.86\textwidth]{fig_pc1_age.png}
...
\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{fig_joint_3d.png}
...
\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{Fig_lasso_n_requerido.png}
...
\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{Fig_beta_posterior.png}
...
\begin{table}[H]
\centering\small
\caption{Required $n$ when component-level evidence is quantified by the default JZS Bayes factor on the partial correlation (two-sided; success = $\ge1$ brain component with $\BF_{H_1/H_0}>u$; $K=20$). ``n.a.'' = not attainable with $n\le160$.}
\label{tab:bfthr}
\begin{tabular}{crrr}
\toprule
True effect $\rho$ & $\BF_{H_1/H_0}>4$ & $\BF_{H_1/H_0}>6$ & $\BF_{H_1/H_0}>10$\\
\midrule
0.2 & n.a. & n.a. & n.a.\\
0.3 & 159 & n.a. & n.a.\\
\rowcolor{RowGrey}
...
The Bayes-factor thresholds are stricter than the credible-interval criterion (Figure~\ref{fig:kbf}): at $\rho=0.4$ the required $n$ rises from $72$ (CrI) to $90$, $97$ and $106$ for $\BF_{H_1/H_0}>4$, $>6$ and $>10$; at $\rho=0.5$ the proposed $n=60$ gives $P=0.796$ for $\BF>4$ (and $n=72$ gives $0.904$), while at $\rho=0.4$ it gives $0.549$ ($0.660$ at $n=72$). In exchange, the family-wise false-detection rate under the null is roughly halved relative to the CrI criterion ($0.13$ vs.\ $0.19$ at $\BF>4$, and $0.05$ at $\BF>10$). Thresholds may therefore be chosen to trade detection power for specificity, and all requirements are recomputable with the released code.

\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{Fig_lasso_K_BF.png}
\caption{Sensitivity analyses. (A) Probability that at least one brain component reaches $\BF_{H_1/H_0}>u$ for $u=4,6,10$ ($K=20$; solid $\rho=0.4$, dashed $\rho=0.3$; red dotted line = global null at $\BF>4$). (B) Required $n$ under the Lasso-CrI criterion as a function of the number of retained components $K$.}
\label{fig:kbf}
\end{figure}

\begin{figure}[H]
\centering\includegraphics[width=0.92\textwidth]{Fig_3D_BF_lasso_c1.png}
...
\midrule
Benchmark: JZS $t$-test $n$/group for $P(\BF_{H_1/H_0}{>}3)\ge0.8$ & 91 & 53 & 34 & 22\\
\bottomrule
\end{tabular}
...
\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{Fig_lasso_cohorte2.png}
...
\subsection{Fixed-design addendum: recovery of the MCI--HC pattern at $n=36$ per group}

A committee question fixes the Cohort-2 design ($n=36$ per group, $K=5$ components) and asks for the probability of recovering an MCI--HC effect equivalent to $d=0.65$--$0.70$. The generative model, priors and analysis are exactly those of the Cohort-2 simulation above; the effect is placed in one true brain component, and two probabilities are reported: \emph{detection} (the pipeline criterion, at least one brain component with a $95\%$ credible interval excluding zero) and \emph{pattern recovery} (the truly discriminating component itself significant). The per-component Bayes-factor criterion ($\BF_{H_1/H_0}$ on the partial correlation of each component with the group; controls: remaining components, hormonal PC1 and age; $m=n-K-1$) extends the answer across evidence thresholds; $400$ Gibbs datasets per cell, $4{,}000$ for the analytic Bayes-factor grid.

\begin{table}[H]
\centering\small
\caption{Probability of detection ($\ge1$ brain component significant) and of pattern recovery (the true component significant) at $n=36$ per group, Bayesian-Lasso criterion ($95\%$ CrI, two-sided), by number of retained components. Family-wise rate = detection probability under the global null.}
\label{tab:focal}
\begin{tabular}{crrrrr}
\toprule
 & \multicolumn{2}{c}{$d=0.65$} & \multicolumn{2}{c}{$d=0.70$} & \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
...
\begin{table}[H]
\centering\small
\caption{Context: required $n$ per group for $0.80$ probability ($K=5$), under both definitions, and the per-component Bayes-factor criterion at the focal point.}
\label{tab:ctx}
\begin{tabular}{lrrrrr}
\toprule
 & $d=0.50$ & $d=0.65$ & $d=0.70$ & $d=0.80$ & $d=1.00$\\
\midrule
Required $n$/group, detection & n.a. & \textbf{66} & \textbf{54} & 47 & 29\\
Required $n$/group, recovery & n.a. & 68 & 54 & 48 & 29\\
\rowcolor{RowGrey}
$P$(true comp.\ $\BF_{H_1/H_0}>4$) at 36/group & 0.22 & 0.42 & 0.51 & 0.66 & 0.88\\
\rowcolor{RowGrey}
$P$(true comp.\ $\BF_{H_1/H_0}>10$) at 36/group & 0.14 & 0.30 & 0.38 & 0.54 & 0.80\\
\bottomrule
\end{tabular}
\end{table}

Three readings follow. First, at the requested design point the pipeline detects the effect in roughly half of the studies ($0.47$ at $d=0.65$, $0.53$ at $d=0.70$) and recovers the true component slightly less often ($0.46$ and $0.52$), so a null result at $n=36$ per group would be uninformative about effects of this size. Reaching the $0.80$ target for detection requires about $66$ per group at $d=0.65$ and $54$ at $d=0.70$ ($68$ and $54$ for recovery of the true component). Second, the answer is essentially flat in $K$ (Table~\ref{tab:focal}): retaining $5$ or $20$ components moves recovery by a few points at most, while the family-wise null rate grows from $0.03$ to $0.16$, so the compact set remains preferable for specificity rather than for power. Third, demanding per-component Bayes factors makes the fixed design harder still: at $36$ per group the probability that the true component clears $\BF_{H_1/H_0}>4$ is $0.42$--$0.51$ in the requested band, and $0.30$--$0.38$ at $\BF_{H_1/H_0}>10$. Including a hormonal MCI--HC difference ($d_h=0.3$) changes little at the focal point (detection $0.47$--$0.58$), with the hormonal coefficient itself rarely significant ($0.09$--$0.08$).

\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{Fig_v35_recuperacion.png}
\caption{Fixed-design addendum. (A) Detection (solid) and pattern recovery (dashed) as a function of $n$ per group at $K=5$; the dotted red line marks $36$ per group. (B) Both probabilities at $36$ per group across $K$. (C) The per-component Bayes-factor criterion at the focal point; the shaded band is $d=0.65$--$0.70$. (D) Specificity under the global null.}
\label{fig:v35}
\end{figure}

\begin{figure}[H]
\centering\includegraphics[width=0.92\textwidth]{Fig_3D_BF_lasso_c2.png}
\caption{The fixed-design question in 3D: probability that at least one brain component clears the $\BF_{H_1/H_0}$ threshold, over $n$ per group and the threshold, one surface per MCI--HC separation $d$. The vertical posts mark the vertices $n=36$ per group at the reference thresholds $\BF_{H_1/H_0}=3$ and $10$.}
\label{fig:v35tresd}
\end{figure}
