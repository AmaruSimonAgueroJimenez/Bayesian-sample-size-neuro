...
: Left: sensitivity of the required $n$ to the number of retained components (Lasso-CrI criterion; family-wise rate under the null at $n=72$). Right: required $n$ when success demands $\ge1$ component with $\mathrm{BF}_{H_1/H_0}$ above a threshold ($K=20$).
:::
...
![Cohort 2 in 3D: probability that at least one brain component clears the per-component $\mathrm{BF}_{H_1/H_0}$ threshold, over $n$ per group and the threshold, one surface per MCI--HC separation $d$ ($K=5$, exposure in the model). Vertical posts mark $36$ per group at $\mathrm{BF}_{H_1/H_0}=3$ and $10$.](../results/pm25/Fig_3D_BF_lasso_c2.png){#fig:tresd_c2 width="92%"}
