...
\textbf{Response.} $Y$ is the standardized long-term residential \pmf{} exposure index. Each participant's exposure is the 2013--2022 mean \pmf{} concentration of their residential comuna from the ACAG satellite-derived annual surface; after normalizing comuna names in the recruitment database, $540$ of $601$ valid records were assigned an exposure, spanning $52$ comunas. The participant-level distribution has mean $24.3$ \ugm, SD $3.2$, range $12.7$--$27.8$ and skewness $-1.08$: most participants live in high-exposure Santiago comunas, with a thinner low-exposure tail (Figure~\ref{fig:calib}A--B). In every iteration, $N$ observations are resampled with replacement from this empirical pool of $540$ standardized values, so the simulation inherits the true, strongly left-skewed shape of the exposure distribution. The repository's candidate exposure metrics (long-term mean, mean peak level, hours above $50$ \ugm, cumulative dose above $50$, episode counts) correlate $0.82$--$0.95$ at the comuna level (Figure~\ref{fig:calib}C); they form essentially one dimension, so the scalar long-term mean is used as the exposure index and effect sizes below are per SD of exposure ($1$ SD $=3.2$ \ugm{} in this cohort).
...
\rowcolor{RowGrey}
5 & $R^2=0.16$ & \textbf{127} & 133 & 140\\
\rowcolor{RowGrey}
5 & $R^2=0.25$ & \textbf{70} & 76 & 82\\
...
\begin{figure}[H]
\centering\includegraphics[width=\textwidth]{pm25/Fig_v4_cohorte2.png}
\caption{Cohort 2 results. (A) Power curves at $\BF_{10}\ge10$ with $K=5$ brain components plus the \pmf{} exposure, by brain separation (Mahalanobis $D$); the dotted grey line is the global null. (B) Number of components at $R^{2}=0.16$, with (dashed) and without (solid) an MCI--HC exposure effect of $d_h=0.3$.}
\label{fig:v4c2}
\end{figure}
...
\texttt{data/calibracion\_pm25.csv} & Empirical $Y$ pool: participant-level residential exposure, built from \texttt{pm25\_anual\_comunal\_acag.csv} and the recruitment database\\
